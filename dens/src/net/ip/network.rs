...
pub struct Network {
    ...
    ipv4_generator: generator::Ipv4,
    ipv6_generator: generator::Ipv6,
    // blocking, blocked
...
impl Network {
    ...
    pub fn add_machine(&mut self, host: &impl HasNic) -> (Ipv4Addr, Ipv6Addr) {
        self.network.add_machine(host);
        let generated_ipv4 = self.ipv4_generator.next();
        let generated_ipv6 = self.ipv6_generator.next();
