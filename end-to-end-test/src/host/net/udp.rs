...
use crate::host::io::interest::Interest;
use crate::host::io::ready::Ready;
...
impl UdpSocket {
    ...
    pub async fn bind<A: ToSocketAddrs>(addrs: A) -> io::Result<UdpSocket> {
        let addrs = to_socket_addrs(addrs)?;
    ...
    fn bind_addr(addr: SocketAddr) -> io::Result<UdpSocket> {
        let recv = OS.with(|os| os.borrow_mut().bind_to_addr(addr))?;
        Ok(Self {
            io: recv,
            local_addr: addr,
    ...
    pub async fn connect<A: ToSocketAddrs>(&self, addr: A) -> io::Result<()> {
        ...
        for addr in addrs {
            *self.peer_addr.borrow_mut() = Some(addr);
            return Ok(());
        }
    ...
    /// can be used to concurrently `recv` / `send` to the same socket on a
    /// single task without splitting the socket.
    ///
    /// The function may complete without the socket being ready. This is a
    /// false-positive and attempting an operation will return with
    /// `io::ErrorKind::WouldBlock`. The function can also return with an empty
    /// [`Ready`] set, so you should always check the returned value and
    /// possibly wait again if the requested states are not set.
    ///
    /// # Cancel safety
    ///
    ...
    pub async fn ready(&self, interest: Interest) -> io::Result<Ready> {
        if self.peer_addr.borrow().is_none() {
            Err(ErrorKind::NotConnected)?;
        }
        // if is closed and peer_addr is set then it was aborted by peer
        if self.io.is_closed() {
            Err(ErrorKind::ConnectionAborted)?;
        }
        // is always writable if the socket isn't closed
        if interest.is_writable() {
            return Ok(Ready::WRITABLE);
        }
        if interest.is_readable() {
            while self.io.is_empty() {
                let dur = CONFIG.with(|v| v.tick_amount());
                // simply sleep for one tick
    ...
    pub fn try_send(&self, buf: &[u8]) -> io::Result<usize> {
        let Some(peer_addr) = *self.peer_addr.borrow() else {
            Err(ErrorKind::NotConnected)?
        };
        OS.with(|os| os.borrow().udp_network().try_send_to(peer_addr));
    }

    /// Waits for the socket to become readable.
    ///
    /// This function is equivalent to `ready(Interest::READABLE)` and is
    /// usually paired with `try_recv()`.
    ///
    ...
    /// bytes read.
    ///
    /// This method must be called with valid byte array `buf` of sufficient
    ...
    /// Tries to send data on the socket to the given address, but if the send
    /// is blocked this will return right away.
    ///
    ...
    ///
    /// # Notes
    /// Note that the socket address **cannot** be implicitly trusted, because
    /// it is relatively trivial to send a UDP datagram with a spoofed
    /// origin in a [packet injection attack]. Because UDP is stateless and
    /// does not validate the origin of a packet, the attacker does not need
    /// to be able to intercept traffic in order to interfere.
    /// It is important to be aware of this when designing your
    ...
    /// Tries to read or write from the socket using a user-provided IO
    /// operation.
    ///
    ...
    /// The closure should perform only one type of IO operation, so it should
    /// not require more than one ready state. This method may panic or
    /// sleep forever if it is called with a combined interest.
    ///
    /// Usually, [`readable()`], [`writable()`] or [`ready()`] is used with this
    /// function.
    ///
    ...
    /// Receives a single datagram from the connected address without removing
    /// it from the queue. On success, returns the number of bytes read from
    ...
    ///
    /// If you're merely interested in learning the sender of the data at the
    /// head of the queue, try [`peek_sender`].
    ///
    /// Note that the socket address **cannot** be implicitly trusted, because
    /// it is relatively trivial to send a UDP datagram with a spoofed
    /// origin in a [packet injection attack]. Because UDP is stateless and
    ...
    /// Receives data from the connected address, without removing it from the
    /// input queue.
    ///
    /// # Notes
    ///
    ...
    ///
    /// If you're merely interested in learning the sender of the data at the
    /// head of the queue, try [`poll_peek_sender`].
    ///
    /// Note that the socket address **cannot** be implicitly trusted, because
    /// it is relatively trivial to send a UDP datagram with a spoofed
    /// origin in a [packet injection attack]. Because UDP is stateless and
    ...
    /// Tries to receive data on the connected address without removing it from
    /// the input queue. On success, returns the number of bytes read.
    ///
    /// When there is no pending data, `Err(io::ErrorKind::WouldBlock)` is
    /// returned. This function is usually paired with `readable()`.
    ///
    ...
    ///
    /// If you're merely interested in learning the sender of the data at the
    /// head of the queue, try [`try_peek_sender`].
    ///
    /// Note that the socket address **cannot** be implicitly trusted, because
    /// it is relatively trivial to send a UDP datagram with a spoofed
    /// origin in a [packet injection attack]. Because UDP is stateless and
    ...
    ///
    /// On Windows, if the data is larger than the buffer specified, the buffer
    /// is filled with the first part of the data, and `peek_from` returns the
    /// error `WSAEMSGSIZE(10040)`. The excess data is lost.
    /// Make sure to always use a sufficiently large buffer to hold the
    /// maximum UDP packet size, which can be up to 65536 bytes in size.
    ///
    /// MacOS will return an error if you pass a zero-sized buffer.
    ...
    /// Tries to receive data on the socket without removing it from the input
    /// queue. On success, returns the number of bytes read and the sending
    ...
    /// Retrieve the sender of the data at the head of the input queue, waiting
    /// if empty.
    ///
    /// This is equivalent to calling [`peek_from`] with a zero-sized buffer,
    /// but suppresses the `WSAEMSGSIZE` error on Windows and the "invalid
    /// argument" error on macOS.
    ///
    /// Note that the socket address **cannot** be implicitly trusted, because
    /// it is relatively trivial to send a UDP datagram with a spoofed
    /// origin in a [packet injection attack]. Because UDP is stateless and
    ...
    ///
    /// This is equivalent to calling [`poll_peek_from`] with a zero-sized
    /// buffer, but suppresses the `WSAEMSGSIZE` error on Windows and the
    /// "invalid argument" error on macOS.
    ///
    /// # Notes
    ///
    /// Note that on multiple calls to a `poll_*` method in the `recv`
