...
impl<const N: usize> Display for Distance<N> {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        ...
        f.debug_tuple("Distance")
            .field(&from_fn(|f| as_id.display_id_bytes(f)))
            .finish()
    }
}

#[derive(Clone)]
pub struct Pair<Node: Eq, const ID_LEN: usize>(Distance<ID_LEN>, Node);
...
impl<Node: Eq + Debug, const ID_LEN: usize> Debug for Pair<Node, ID_LEN> {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
